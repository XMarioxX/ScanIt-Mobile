...
class MongoClienteApartadoRepository(ClienteApartadoRepository):
    ...
    def get_all(self) -> List[ClienteApartado]:
        return [ClienteApartado(**{**clienteApartado, "id": str(clienteApartado["_id"])}) for clienteApartado in self.collection.find()]
    ...
    def update(self, clienteApartado_id: str, clienteApartado: ClienteApartado) -> bool:
        clienteApartado.fechaUpdate = datetime.now().isoformat()  
        result = self.collection.update_one(
            {"_id": ObjectId(clienteApartado_id)}, {"$set": clienteApartado.dict(exclude_unset=True)}
        )
