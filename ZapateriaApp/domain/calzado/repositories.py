...
class MongoCalzadoRepository(CalzadoRepository):
    ...
    def get_all(self) -> List[Calzado]:
        return [Calzado(**{**calzado, "id": str(calzado["_id"])}) for calzado in self.collection.find()]
    ...
    def update(self, calzado_id: str, calzado: Calzado) -> bool:
        calzado.fechaUpdate = datetime.now().isoformat()  
        result = self.collection.update_one(
            {"_id": ObjectId(calzado_id)}, {"$set": calzado.dict(exclude_unset=True)}
        )
