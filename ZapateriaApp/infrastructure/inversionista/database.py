...
class MongoInversionistaRepository(InversionistaRepository):
    ...
    def get_all(self) -> List[Inversionista]:
        return [
            Inversionista(**{**inversionista, "id": str(inversionista["_id"])}) 
            for inversionista in self.collection.find()
        ]
    ...
    def update(self, inversionista_id: str, inversionista: Inversionista) -> bool:
        result = self.collection.update_one(
            {"_id": ObjectId(inversionista_id)}, 
            {"$set": inversionista.dict(exclude_unset=True)}
        )
