...
class MongoCalzadoEstadoRepository(CalzadoEstadoRepository):
    ...
    def get_all(self) -> List[CalzadoEstado]:
        return [
            CalzadoEstado(**{**calzadoEstado, "id": str(calzadoEstado["_id"])}) 
            for calzadoEstado in self.collection.find()
        ]
    ...
    def update(self, calzadoEstado_id: str, calzadoEstado: CalzadoEstado) -> bool:
        result = self.collection.update_one(
            {"_id": ObjectId(calzadoEstado_id)}, 
            {"$set": calzadoEstado.dict(exclude_unset=True)}
        )
